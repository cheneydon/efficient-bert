...
def get_final_text(pred_text, orig_text, tokenizer):
    ...
    tok_text = ' '.join(tokenizer._basic_tokenize(orig_text))
...
def create_squad_answer_texts(task, tokenizer, all_examples, all_encoded_inputs, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if task == 'squad2.0':
            ...
            predicted_answer_texts[example.qa_id] = nbest[0].text
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            for i in range(start_n_top):
                for j in range(end_n_top):
                    ...
                    # that the start of the span is in the question. We throw out all
                    # invalid predictions.
                    if start_index >= feature.context_len - 1:
                        continue
                    if end_index >= feature.context_len - 1:
                        continue
                    if not feature.is_max_context_token.get(start_index, False):
                        continue
                    if end_index < start_index:
